// Global document click listener for Buy Now buttons
document.addEventListener('click', function(e) {
  const btn = e.target.closest('.select-pkg-btn, .home-buy-btn');
  if (btn) {
    ...
    grid.innerHTML = products.map(p => `
      <div class="product-card" data-pkg="${p.title || 'Product'}" style="background: #FFF; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 15px rgba(0,0,0,0.05); display: flex; flex-direction: column; transition: transform 0.2s, box-shadow 0.2s;">
        ...
        <div class="prod-content" style="padding: 12px; display: flex; flex-direction: column; flex-grow: 1;">
          <h3 class="pkg-title" style="margin: 0 0 6px 0; font-size: 0.85rem; color: #0A2528; line-height: 1.3;">${p.title || 'Unknown Product'}</h3>
          ...
          ${p.description ? `<p style="color: #475569; font-size: 0.75rem; margin-bottom: 12px; line-height: 1.4;">${p.description}</p>` : ''}
          
          <div style="margin-top: auto;">
            <button type="button" class="btn btn-primary pkg-cta-btn select-pkg-btn" style="width: 100%; border-radius: 8px; padding: 8px; background: #00A0A8; color: #FFF; font-weight: 600; font-size: 0.9rem; cursor: pointer; border: none; transition: background 0.3s; margin-top: 6px;" onmouseover="this.style.background='#00858C'" onmouseout="this.style.background='#00A0A8'">Buy Now</button>
          </div>
        </div>
      </div>
    `).join('');
